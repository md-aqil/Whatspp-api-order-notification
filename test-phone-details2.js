...
async function testPhoneDetails() {
  ...
  try {
    ...
    const url = `https://graph.facebook.com/v22.0/${integrations.whatsapp.phoneNumberId}?access_token=${integrations.whatsapp.accessToken}`;
    ...
    if (response.ok) {
      ...
      // Check if business_account_id exists
      if (data.business_account_id) {
        console.log('Business Account ID found:', data.business_account_id);
      } else {
        console.log('Business Account ID not found in response');
      }
    } else {
