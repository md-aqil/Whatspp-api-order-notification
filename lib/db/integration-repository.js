...
import { getLocalIntegrationRecord, saveLocalIntegrationRecord } from '../local-settings-store';
...
export async function getStoredIntegrations(userId = 'default') {
  try {
    ...
    return userId === 'default' ? getLocalIntegrationRecord() : null
  }
}
  ...
  try {
    ...
    if (existing) {
      ...
      await pool.execute(
        ...
        [encrypt(JSON.stringify(data)), existing.id]
      )
    } else {
      ...
      await pool.execute(insertSql, [String(userId || 'default'), encrypt(JSON.stringify(data))])
    }
