...
export async function seedDefaultAutomationsForUser(userId) {
  try {
    ...
    if (existing && existing.length > 0) {
      console.log(`[Automation DB] User ${userId} already has ${existing.length} automations. Skipping seed.`)
      return 
    }
    
    console.log(`[Automation DB] Seeding ${defaultAutomations.length} default automations for user ${userId}...`)
    for (const automation of defaultAutomations) {
      // Use a special flag to indicate this is a seed, so it doesn't overwrite if called accidentally
