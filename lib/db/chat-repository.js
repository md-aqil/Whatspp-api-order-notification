...
export async function saveIncomingMessage(message, userId = 'default') {
  ...
  // Save message ATOMICALLY to handle race conditions
  const affectedRows = await insertStoredMessage({
    id: msgId,
    userId,
    phone,
    message: text,
  ...
  if (affectedRows !== 1) {
    // 0 or 2 means it already existed (update or ignored)
    return { id: msgId, duplicate: true }
  }

  // Upsert chat ONLY if it's a new message
