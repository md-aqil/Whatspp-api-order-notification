...
export async function handleZohoWebhook(request) {
  try {
    ...
    if (Object.keys(body).length > 0) {
      ...
      await insertWebhookLog('zoho', webhookTopic, body)

      // 1. Identify User (Priority: Query Param > Body > Most Recent Integration)
      const url = new URL(request.url)
      let userId = url.searchParams.get('userId') || body.userId
      
      const integrationsRows = await queryMany('SELECT userId FROM integrations ORDER BY updatedAt DESC')
      if (!userId && integrationsRows.length > 0) {
        userId = integrationsRows[0].userId
      }
