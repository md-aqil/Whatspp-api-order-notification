...
export async function handleShopifyWebhook(request) {
  try {
    ...
    if (topic === 'orders/create' && body.id) {
      ...
      // For prepaid orders, trigger immediate order confirmation automation
      // For COD orders, confirmation message is triggered after the customer clicks "Confirm Order" on WhatsApp
      if (!isCod) {
        const automationIntegrations = await getStoredIntegrations(userId)
