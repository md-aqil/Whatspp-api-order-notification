...
function handleMessageEdit(messagingEvent, options, instagramAccountId) {
  if (!messagingEvent?.message_edit) return null;
  const safeOptions = options || {};
  const { messageTextByMid = {}, senderIdByMid = {}, usernameByMid = {}, outboundMessageIds = new Set(), quickReplyPayloadByMid = {} } = safeOptions;
  ...
  if (hydratedText) {
    ...
    const quickReplyPayload = quickReplyPayloadByMid?.[mid];
    ...
    return {
      ...
      context: {
        ...
        _isInteractiveReply: !!quickReplyPayload,
        _chosenOptionId: quickReplyPayload || null,
      },
    };
  }
