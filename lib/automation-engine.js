...
async function runAutomationLoop(
  ...
  if (!currentStepId) {
    ...
    if (!currentStepId && (isIncomingWhatsApp || isIncomingInstagram) && context._isInteractiveReply) {
...
async function getInstagramCredentialsForAccount(userId, instagramAccountId) {
  const row = await queryOne(
    "SELECT accessToken, pageId FROM instagram_accounts WHERE userId = ? AND instagramAccountId = ? LIMIT 1",
