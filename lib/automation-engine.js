...
export async function processAutomationEvent(jobData) {
  ...
  console.log(`[Automation Engine] Credentials check — WA phoneNumberId: ${waPhone ? waPhone.substring(0,8)+'...' : 'MISSING'}, isInstagram: ${isInstagramEvent}`)
  ...
  if (automationId && stepId) {
    ...
    console.log(`[Automation Engine] Found ${rows?.length || 0} active automations for user ${userId}`)
    ...
    for (const automation of automations) {
      ...
      automation._hasKeywords = !!(trigger?.config?.keywords || trigger?.keyword)
    }

    // Sort: Specific keywords first, then catch-alls
    automations.sort((a, b) => (b._hasKeywords ? 1 : 0) - (a._hasKeywords ? 1 : 0))

    let anyTriggered = false
    for (const automation of automations) {
      // If we already triggered a keyword-specific automation, skip catch-alls
      if (anyTriggered && !automation._hasKeywords) {
        console.log(`[Automation Engine] Skipping catch-all automation "${automation.name}" because a more specific flow triggered.`)
        continue
      }

      console.log(`[Automation Engine] Checking automation "${automation.name}" (${automation.id}) for event "${event}"`)
      const didRun = await runAutomationLoop(automation, context, integrations, userId, event)
...
async function runAutomationLoop(automation, context, integrations, userId, eventType, startFromStepId = null) {
  ...
  const isIncomingInstagram = eventType === 'instagram.message_received' || eventType === 'instagram.comment_created'
  
  const recipient = normalizeRecipient(resolveRecipient({ recipientMode: 'customer' }, context), context?.platform)
  if (!recipient) return
  ...
  if (!currentStepId) {
    ...
    if ((isIncomingWhatsApp || isIncomingInstagram) && keywordString) {
      ...
      const rawMsg = context.customer_message || context.messageText || context.commentText || ''
  ...
  while (currentStepId && !visited.has(currentStepId)) {
    ...
    if (step.type === 'message' || step.type === 'interactive') {
      const isIncomingInstagram = eventType && eventType.startsWith('instagram.')
      const usesApprovedTemplate = Boolean(step.template || step.templateName || step.config?.template || step.config?.templateName)
      if (!isIncomingWhatsApp && !isIncomingInstagram && !usesApprovedTemplate) {
...
async function handleGoogleSheetsActionStep(step, context, userId) {
  ...
  if (!sheetClient) {
    console.warn(`[Automation Engine] Google Sheets integration not found for user ${userId}. Skipping action.`)
    return
  }

  try {
    if (action === 'append_row') {
      const spreadsheetId = step.spreadsheetId || step.config?.spreadsheetId || sheetClient.data.defaultSettings?.spreadsheetId
      const sheetName = step.sheetName || step.config?.sheetName || sheetClient.data.defaultSettings?.sheetName || 'Sheet1'
      
      if (!spreadsheetId) {
        console.warn(`[Automation Engine] Spreadsheet ID missing for Google Sheets action.`)
        return
      }
      ...
      const rowValues = columns.map(col => {
        const rawVal = typeof col === 'object' ? (col.value || col.val || '') : String(col)
        return interpolate(rawVal, context)
      })

      if (rowValues.length === 0) {
        // Fallback standard row: Phone, Name, Message, Timestamp, Platform
        const phone = context.customerPhone || context.customer_phone || context.phone || context.from || ''
      ...
      console.log(`[Automation Engine] Google Sheets Append Row Success for sheet: ${sheetName}`)
    }
  } catch (err) {
    console.error(`[Automation Engine] Google Sheets Action Error (${action}):`, err.message)
  }
}
...
function buildZohoLeadPayload(step, context) {
  ...
  const createFields = compactZohoFields({
    ...
    First_Message_At: formatToZohoDateTime(configuredCreateFields.First_Message_At) || formatToZohoDateTime(context.first_message_at) || zohoMessageTime,
    Last_Inbound_Message_At: formatToZohoDateTime(configuredCreateFields.Last_Inbound_Message_At) || zohoMessageTime,
  ...
  const updateFields = compactZohoFields({
    ...
    Last_Inbound_Message_At: formatToZohoDateTime(configuredUpdateFields.Last_Inbound_Message_At) || zohoMessageTime,
...
async function handleMessageStep(step, context, integrations, recipient) {
  ...
  if (context.platform === 'instagram') {
    try {
      const igToken = await getInstagramAccessTokenForAccount(context.userId || 'default', context.instagramAccountId)
      if (!igToken) {
      ...
      if (context.commentId) {
        const commentReplyText = step.config?.commentReply || `Sent you a DM, @${context.username}! Check your inbox 📥✨`
        ...
        try {
          ...
          console.log(`[Automation Engine] Instagram comment public reply sent successfully to comment ${context.commentId}`)
        } catch (commentErr) {
          console.error('[Automation Engine] Failed to reply to Instagram comment:', commentErr.message)
        }
      }

      const pdfUrl = step.config?.pdfUrl || step.config?.fileUrl || null
      const res = await sendInstagramDM(igToken, recipient, body, pdfUrl)
      
      if (res?.message_id) {
        return { success: true, wamid: res.message_id }
      }
      if (res?.error) {
        return { success: false, error: res.error.message || 'Instagram API Error' }
      }
  ...
  if (step.type === 'interactive') {
    ...
    if (rawButtons.length === 0) {
      ...
      const rows = rawButtons.slice(0, 10).map(b => ({
        id: b.id || b.reply?.id || 'opt_' + Math.random().toString(36).substr(2, 5), 
        title: (b.label || b.title || b.reply?.title || 'Option').substring(0, 24)
      }))
...
async function getInstagramAccessTokenForAccount(userId, instagramAccountId) {
  const row = await queryOne(
    'SELECT accessToken FROM instagram_accounts WHERE userId = ? AND instagramAccountId = ? LIMIT 1',
    [userId, instagramAccountId]
  )
  return row?.accessToken || null
}

async function sendInstagramDM(accessToken, recipientId, text, pdfUrl = null) {
  const startTime = Date.now()
  ...
  console.log(`[Instagram Send DM] POST ${url} → recipient: ${recipientId}, pdf: ${!!pdfUrl}`)
  ...
  try {
    ...
    if (!res.ok || result.error) {
      console.error(`[Instagram DM] API ERROR (${res.status}):`, JSON.stringify(result.error || result))
      metricsService.incrementCounter('instagram_messages_total', { status: 'error' })
      return result
    }
    
    console.log(`[Instagram DM] SUCCESS — message_id: ${result.message_id}, latency: ${latency}ms`)
    metricsService.incrementCounter('instagram_messages_total', { status: 'success' })
    return result
  } catch (err) {
...
async function replyToInstagramComment(accessToken, commentId, text) {
  ...
  try {
    ...
    if (!res.ok || result.error) {
      console.error(`[Instagram Comment] API ERROR (${res.status}):`, JSON.stringify(result.error || result))
      metricsService.incrementCounter('instagram_comments_total', { status: 'error' })
      return result
    }
    
    console.log(`[Instagram Comment] SUCCESS — comment_id: ${result.id}, latency: ${latency}ms`)
    metricsService.incrementCounter('instagram_comments_total', { status: 'success' })
    return result
  } catch (err) {
