  ...
  if (!currentStepId) {
    ...
    if (!currentStepId && isIncomingWhatsApp && context._isInteractiveReply) {
      ...
      if (awaitingId) {
        ...
        if (currentStepId) {
          console.log(
            `[Automation Engine] Interactive branch resolved: ${currentStepId}`,
          );
          state = await saveAutomationConversationState(
            automation.id,
            recipient,
      ...
      if (!currentStepId) {
        for (const s of steps.filter(
          (step) =>
            ...
            (step.type === "message" && step.connections),
        )) {
          const branchId = resolveInteractiveBranch(
            s,
            context._chosenOptionId,
            context.customer_message,
          );
          if (branchId) {
            ...
            console.log(
              `[Automation Engine] Deep branch match found in step ${s.id}`,
            );
            break;
          }
        }
...
function buildZohoLeadPayload(step, context) {
  ...
  const createFields = compactZohoFields({
    ...
    WhatsApp_Number:
      ...
      context.from,
    Lead_Source:
      configuredCreateFields.Lead_Source || context.lead_source || "WhatsApp",
...
async function handleMessageStep(step, context, integrations, recipient) {
  ...
  if (context.platform === "instagram") {
    try {
      const igToken = await getInstagramAccessTokenForAccount(
      ...
      if (context.commentId) {
        ...
        try {
          ...
          console.log(
            `[Automation Engine] Instagram comment public reply sent successfully to comment ${context.commentId}`,
          );
        } catch (commentErr) {
          console.error(
...
async function sendInstagramDM(accessToken, recipientId, text, pdfUrl = null) {
  ...
  console.log(
    `[Instagram Send DM] POST ${url} → recipient: ${recipientId}, pdf: ${!!pdfUrl}`,
  );
  ...
  try {
    ...
    if (!res.ok || result.error) {
      console.error(
        `[Instagram DM] API ERROR (${res.status}):`,
        JSON.stringify(result.error || result),
      );
