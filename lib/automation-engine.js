...
async function runAutomationLoop(automation, context, integrations, userId, eventType, startFromStepId = null) {
  ...
  if (!currentStepId) {
    ...
    if (isIncomingWhatsApp && keywordString) {
    ...
    // IMPORTANT: If this is a WhatsApp message, only trigger here if NO keywords were defined.
    ...
    if (!currentStepId && trigger && trigger.event === eventType) {
      if (isIncomingWhatsApp && hasKeywords) {
