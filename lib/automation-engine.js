...
async function handleMessageStep(step, context, integrations, recipient) {
  const body = interpolate(step.message || step.config?.body || '', context)
...
function resolveRecipient(step, context) {
  ...
  return context.customerPhone || context.from
}

function interpolate(text, context) {
  return text.replace(/\{\{(.*?)\}\}/g, (_, p) => context[p.trim()] || '')
}
