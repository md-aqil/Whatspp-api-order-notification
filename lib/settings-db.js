...
export async function ensureSettingsTables() {
  if (!settingsTablesReadyPromise) {
    settingsTablesReadyPromise = (async () => {
      ...
      try {
        const [columns] = await pool.query("SHOW COLUMNS FROM integrations LIKE 'googleSheets'")
        if (!columns || columns.length === 0) {
          console.log("Migrating integrations table: adding googleSheets column")
          await pool.query("ALTER TABLE integrations ADD COLUMN googleSheets JSON AFTER zoho")
        }
      } catch (err) {
