...
export function AutomationStudio() {
  ...
  const [botSimulatorOpen, setBotSimulatorOpen] = useState(true)
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-[#0b0d14] text-white overflow-hidden" role="application" aria-label="Flow Studio — Automation Canvas" style={{ fontFamily: 'Inter,system-ui,sans-serif' }}>
          ...
          {viewMode === 'editor' && active && (
            ...
            {viewMode === 'editor' && (
              <>
                <Button
                  size="sm"
                  aria-label="Open Live Bot Simulator"
                  onClick={() => setBotSimulatorOpen(!botSimulatorOpen)}
                ...
                >
                  <Smartphone className="mr-1.5 h-3.5 w-3.5 text-emerald-400" />
                  {botSimulatorOpen ? 'Hide Bot' : 'Live Bot'}
                </Button>
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        <aside aria-label="Node properties" className={`transition-all duration-300 border-l border-white/[0.06] bg-[#0d0f17] flex flex-col ${rightSidebarOpen ? 'w-[296px]' : 'w-0 overflow-hidden border-none'}`}>
          ...
          {/* LIVE BOT PHONE SIMULATOR SIDE-DRAWER */}
          {botSimulatorOpen && (
            <LiveBotPhoneSimulator 
              active={active} 
...
function LiveBotPhoneSimulator({ active, onClose }) {
  const [messages, setMessages] = useState([])
  const [inputText, setInputText] = useState('')
  const [isTyping, setIsTyping] = useState(false)
  const chatBottomRef = useRef(null)

  // Initialize simulation with the active flow
  const initSimulation = useCallback(() => {
    const interactiveStep = active?.steps?.find(s => s.type === 'interactive')
    const welcomeMsg = interactiveStep?.message?.replace(/\{\{customer_name\}\}/g, 'John') || 
      'Hello John, welcome to our store! 👋\n\nHow can we assist you today? Please select an option below:'
    
    const options = interactiveStep?.options || [
      { id: 'opt0', label: '📦 Order Status' },
      { id: 'opt1', label: '👗 Shop Collection' },
      { id: 'opt2', label: '❓ Help & FAQs' },
      { id: 'opt3', label: '💬 Talk to Specialist' }
    ]
  ...
  const handleOptionClick = (label) => {
    ...
    setTimeout(() => {
      setIsTyping(false)
      if (label.includes('Order Status') || label.includes('Latest Order')) {
        setMessages(prev => [
          ...prev,
          {
            ...
            card: {
              ...
              progress: 80,
              items: '1x Premium Leather Jacket (Size L, Black)',
              total: '$89.00',
              buttons: ['📍 Live Courier Tracking', '💬 Need Help']
            },
            timestamp: 'Just now'
          }
        ])
      } else if (label.includes('Enter Order ID') || label.includes('Search')) {
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            sender: 'bot',
            text: '🔍 *Search Specific Order*\n\nPlease reply with your **Order ID** (e.g. #1042 or 1042), and I will pull up the live status immediately! ✨',
            timestamp: 'Just now'
          }
        ])
      } else if (label.includes('Shop Collection') || label.includes('Shop')) {
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            sender: 'bot',
            text: 'Great! Are you looking for men\'s, women\'s, or accessories today? 🛍️',
            buttons: ['👚 Women\'s Collection', '👔 Men\'s Collection', '🎒 Accessories'],
            timestamp: 'Just now'
          }
        ])
      } else if (label.includes('Help') || label.includes('FAQ')) {
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            ...
            buttons: ['🔄 Returns Policy', '🚚 Shipping Times', '📍 Store Location'],
            timestamp: 'Just now'
          }
        ])
      } else if (label.includes('Specialist') || label.includes('Support')) {
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            sender: 'bot',
            text: '👨‍💻 I\'m connecting you with a member of our specialist team right now.\n\nPlease stay tuned—an agent will reply in a few minutes!',
            timestamp: 'Just now'
          }
        ])
        ...
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            sender: 'bot',
            text: '✨ We offer a hassle-free 30-day return policy on all unworn items with original tags attached!',
            timestamp: 'Just now'
          }
        ])
        ...
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            sender: 'bot',
            text: `You selected "${label}". How else can we help you today? ✨`,
  ...
  const handleSendText = (e) => {
    ...
    setTimeout(() => {
      ...
      if (orderMatch || text.toLowerCase().includes('order') || text.toLowerCase().includes('track') || text.toLowerCase().includes('where')) {
        ...
        setMessages(prev => [
          ...prev,
          {
            id: String(Date.now() + 1),
            sender: 'bot',
            text: `Thanks for your message: "${text}"! 👋\n\nHow can we assist you? Please choose an option:`,
  ...
  return (
    <aside className="w-[360px] shrink-0 border-l border-white/[0.08] bg-[#0c0d14] flex flex-col h-full shadow-2xl relative z-40 animate-in slide-in-from-right-2 duration-300">
      {/* PHONE SIMULATOR HEADER */}
      <div className="p-3.5 border-b border-white/[0.08] bg-[#11131c] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="h-8 w-8 rounded-full bg-emerald-600/20 border border-emerald-500/30 flex items-center justify-center text-emerald-400 font-black text-xs">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40">Real-time WhatsApp Experience</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col bg-[#0b141a] bg-[radial-gradient(#ffffff05_1px,transparent_1px)] [background-size:16px_16px] overflow-hidden">
        {/* WhatsApp App Bar */}
        <div className="bg-[#1f2c34] px-3.5 py-2.5 flex items-center gap-2.5 border-b border-white/5 shrink-0 shadow-sm">
          <div className="h-7 w-7 rounded-full bg-emerald-600 flex items-center justify-center text-white font-bold text-[11px] shadow">
            VF
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-xs font-bold text-white truncate flex items-center gap-1">
              Vaclav Fashion
              <CheckCircle2 className="w-3 h-3 text-emerald-400 fill-emerald-400 text-black shrink-0" />
            </div>
            <div className="text-[9.5px] text-emerald-400/90 font-medium">Online • Instant replies</div>
          </div>
          <span className="text-[9px] bg-white/10 text-white/60 px-2 py-0.5 rounded-full font-mono font-medium">Live</span>
        </div>

        {/* Message Feed */}
        <div className="flex-1 p-3.5 space-y-3 overflow-y-auto scrollbar-thin">
          <div className="text-center my-1">
            <span className="text-[9px] bg-[#182229] text-white/40 px-2.5 py-1 rounded-md shadow-sm border border-white/5 font-medium">
              TODAY • ENCRYPTED BOT SESSION
            </span>
          </div>
          ...
          {messages.map(msg => (
            ...
            >
              ...
              {msg.text && (
                <div
                  className={`max-w-[88%] rounded-2xl p-3 text-[12px] leading-relaxed shadow-md ${
                    msg.sender === 'user'
                ...
                >
                  ...
                  <div className={`text-[9px] mt-1 text-right ${msg.sender === 'user' ? 'text-emerald-200/60' : 'text-white/30'}`}>
                    {msg.timestamp}
                  </div>
                </div>
              )}

              {/* Order Status Card Simulation */}
              {msg.card && msg.card.type === 'order_status' && (
                <div className="max-w-[94%] bg-[#202c33] border border-emerald-500/30 rounded-2xl rounded-tl-xs p-3.5 text-white shadow-lg space-y-2.5">
                  <div className="flex items-center justify-between border-b border-white/10 pb-2">
                    <div className="flex items-center gap-1.5 font-bold text-xs text-white">
                      <span>📦</span> Order #{msg.card.orderNumber}
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-wider bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full border border-emerald-500/30">
                      {msg.card.status}
                    </span>
                  </div>
                  ...
                  {/* Progress Bar */}
                  <div className="space-y-1">
                    <div className="flex justify-between text-[9.5px] text-white/60 font-medium">
                      <span>Placed</span>
                      <span>Shipped</span>
                  ...
                  <div className="bg-black/30 p-2.5 rounded-xl border border-white/5 space-y-1 text-[11px]">
                    <div className="text-white/80 font-medium">{msg.card.items}</div>
                    <div className="text-white/40 text-[10px]">Total: <strong className="text-emerald-300 font-bold">{msg.card.total}</strong> • Paid via Shopify</div>
                  </div>
                  ...
                  <div className="text-[9px] text-white/30 text-right">Just now</div>
                </div>
              )}
                ...
                <div className="w-full max-w-[88%] mt-2 space-y-1.5">
                  {msg.buttons.map((btnLabel, idx) => (
                    <button
                      ...
                      type="button"
                      onClick={() => handleOptionClick(btnLabel)}
                      className="w-full py-2 px-3 bg-[#182229] hover:bg-[#222e35] active:scale-[0.98] border border-emerald-500/20 hover:border-emerald-400/40 rounded-xl text-emerald-300 text-xs font-bold transition-all shadow flex items-center justify-center gap-1.5 text-center"
                    >
                      {btnLabel}
                    </button>
                  ))}
          ...
          {/* Typing Indicator */}
          {isTyping && (
            <div className="flex items-center gap-1 bg-[#202c33] border border-white/5 py-2 px-3 rounded-2xl rounded-tl-xs w-16 shadow-md">
        ...
        {/* WhatsApp Chat Input */}
        <form onSubmit={handleSendText} className="p-2.5 bg-[#1f2c34] border-t border-white/5 flex items-center gap-2 shrink-0">
          <input
            type="text"
            placeholder="Type a message or #1042..."
            value={inputText}
            onChange={e => setInputText(e.target.value)}
            className="flex-1 bg-[#2a3942] border border-transparent focus:border-emerald-500/50 rounded-2xl px-3.5 py-2 text-xs text-white placeholder:text-white/30 outline-none"
          />
          <Button
            type="submit"
            size="icon"
            disabled={!inputText.trim()}
            className="h-8 w-8 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white shrink-0 shadow disabled:opacity-30"
          >
