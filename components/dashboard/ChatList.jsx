...
export function ChatList({ 
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-[#0b0d14] border-r border-gray-100 dark:border-slate-800">
      {/* List Header */}
      <div className="flex items-center justify-between px-3.5 py-3 border-b border-gray-100 dark:border-slate-800 bg-[#f9fafb] dark:bg-[#111827]/50">
        <div>
          <h2 className="text-base font-black text-gray-900 dark:text-white tracking-tight">Messages</h2>
          <span className="text-[10px] font-medium text-gray-400">
            {chats.length} contact{chats.length !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        <div className="flex items-center space-x-1">
          ...
          <Button
            ...
            className={`h-7 px-2 text-[11px] font-bold rounded-lg transition-all ${
              isMultiSelect 
          ...
          <Dialog open={showNewChatDialog} onOpenChange={setShowNewChatDialog}>
            <DialogTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500/10">
      ...
      {isMultiSelect && (
        <div className="px-3.5 py-2 bg-emerald-50/80 dark:bg-emerald-950/30 border-b border-emerald-100 dark:border-emerald-800/40 flex items-center justify-between animate-in slide-in-from-top-1">
      ...
      {/* Search */}
      <div className="p-2.5 border-b border-gray-50 dark:border-slate-800">
        <div className="relative group">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 group-focus-within:text-emerald-500 transition-colors" />
          <Input 
            placeholder="Search..." 
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="pl-8 h-8 bg-gray-50 dark:bg-slate-800/50 border-none focus-visible:ring-emerald-500 rounded-lg text-xs placeholder:text-gray-400"
          />
        </div>
      </div>
        ...
        {filteredChats.map((chat) => {
          ...
          return (
            <div
              ...
              className={`w-full flex items-center px-3 py-2.5 transition-all duration-150 border-b border-gray-50 dark:border-slate-800/50 cursor-pointer ${
                isMultiSelect
                  ? isSelected
            ...
            >
              ...
              <div className="relative flex-shrink-0">
                <div className="w-9 h-9 rounded-xl overflow-hidden shadow-2xs border border-gray-100 dark:border-slate-700 bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center text-white font-black text-xs uppercase tracking-wider select-none">
                ...
                <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-500 border-2 border-white dark:border-[#0b0d14] rounded-full"></div>
              </div>
              
              <div className="ml-2.5 flex-1 min-w-0 text-left">
                <div className="flex justify-between items-baseline mb-0.5">
                  <h3 className={`font-bold truncate tracking-tight text-xs ${
                    isSelected ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-900 dark:text-white'
                  }`}>
                    {chat.name || chat.phone}
                  </h3>
                  <span className="text-[9px] font-semibold text-gray-400 dark:text-gray-500 whitespace-nowrap ml-1.5">
                ...
                <div className="flex justify-between items-center">
                  ...
                  {chat.unread > 0 && !isMultiSelect && (
                    <span className="ml-1.5 bg-emerald-500 text-white text-[9px] font-black px-1.5 py-0.2 rounded-full shadow-2xs">
                      {chat.unread}
                    </span>
                  )}
                </div>
