...
export function Sidebar({ sidebarOpen, setSidebarOpen, user }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setSidebarOpen(false)}
                >
                  <span className={`flex h-9 w-9 items-center justify-center rounded-lg transition-all ${isActive
