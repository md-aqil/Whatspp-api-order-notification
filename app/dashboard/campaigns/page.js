...
function CampaignsStudio() {
  ...
  const templateSupportsProducts = useMemo(() => {
    if (messageMode === 'custom') return true
    if (!selectedTemplate) return false
    
    // Check if template has an image/video/document header
    const headerComponent = selectedTemplate.components?.find(c => c.type === 'HEADER')
    const hasMediaHeader = Boolean(headerComponent && (headerComponent.format === 'IMAGE' || headerComponent.format === 'VIDEO' || headerComponent.format === 'DOCUMENT'))
    
    // Check if template has a CATALOG or MPM button
    const buttonsComponent = selectedTemplate.components?.find(c => c.type === 'BUTTONS')
    const hasCatalogButton = Array.isArray(buttonsComponent?.buttons) && buttonsComponent.buttons.some(b => {
      const bType = String(b?.type || '').toUpperCase()
      return bType === 'CATALOG' || bType === 'MPM'
    })

    // Check if template variables contain product tags
    const hasProductVariable = templateVariables.some(v => 
      typeof v === 'string' && (v.includes('product') || v.includes('catalog'))
    )

    return hasMediaHeader || hasCatalogButton || hasProductVariable
  }, [messageMode, selectedTemplate, templateVariables])
